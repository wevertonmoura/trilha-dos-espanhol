...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!senhaCorreta || senha !== senhaCorreta) {
      console.error("Acesso bloqueado ao tentar excluir: Senha inválida.");
      return res.status(401).json({ error: 'Acesso negado' });
    }

    // 🛡️ TRAVA 2: Garantir que o ID foi enviado
    if (!id) {
      return res.status(400).json({ error: 'ID não fornecido para exclusão.' });
    }

    // Exclusão na tabela correta
    const { error } = await supabase.from('participantes').delete().eq('id', id);
    
    if (error) {
      console.error("Erro interno do Supabase ao excluir:", error.message);
    ...
    console.error("Erro no servidor da Vercel ao excluir:", err);
